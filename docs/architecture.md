...
## 컨트롤 플레인(마스터 노드)
컨트롤 플레이는 클러스터 기능을 제어하고 전체 클러스터가 동작하게 만드는 역할을 합니다.
구성 요소는 다음고 같습니다.
- etcd(분산 저장 스토리지)
- API 서버
- 스케줄러
...
이 구성 요소들은 클러스터 상태를 저장하고 관리하지만, 애플리케이션 컨테이너를 직접 실행하진 않습니다.
...
### API 서버
- 다른 모든 구성 요소와 통신하며 RESTful API를 통해 상태에 대해 CRUD 작업을 수행하며, etcd에 데이터를 저장합니다.
...
리소스버전이 일치하지 않은 경우 내용은 거부되고(409 status code)를 반환합니다.
...
레플리카셋 컨트롤러는 레플리카셋이 생성되면, 정의된 레플리카 수와 파드 셀렉터에 따라 필요한 수의 파드를 파드 템플릿 기반으로 생성합니다.
