...
- [파드](#파드)
  - [파드 기본 개념](#파드-기본-개념)
  - [파드가 필요한 이유](#파드가-필요한-이유)
  - [파드 내부](#파드-내부)
  - [yaml 매니페스트 구조](#yaml-매니페스트-구조)
  - [파드 로그](#파드-로그)
  - [파드 요청(port-forward)](#파드-요청port-forward)
  - [파드 삭제](#파드-삭제)
  - [파드 상태](#파드-상태)
  - [파드 컨디션](#파드-컨디션)
- [레이블](#레이블)
...
- [컨테이너](#컨테이너)
  - [초기화 컨테이너](#초기화-컨테이너)
  - [초기화 컨테이너 다양한 케이스](#초기화-컨테이너-다양한-케이스)
  - [컨테이너 리소스 제한](#컨테이너-리소스-제한)
  - [ImagePullPolicy](#imagepullpolicy)
  - [라이브니스 프로브](#라이브니스-프로브)
  - [레디니스 프로브](#레디니스-프로브)
  - [컨테이너 상태](#컨테이너-상태)
  - [컨테이너 라이프사이클 훅](#컨테이너-라이프사이클-훅)
- [리소스](#리소스)
  - [쿠버네티스에서 리소스를 관리하는 방법](#쿠버네티스에서-리소스를-관리하는-방법)
  - [cgroup v2](#cgroup-v2)
- [쿠버네티스](#쿠버네티스)
...
`1000MB`를 할당하고 다시 메모리 사용량을 조회한 결과입니다.
```
# kubectl top pod memory-hog
NAME         CPU(cores)   MEMORY(bytes)   
...
```
쿠버네티스 공식문서에 의하면 CPU 제한을 초과하는 요청이 온 경우, 초과하는 요청을 대기 상태로 만듭니다(다음 스케줄링 까지 대기)
이 후 다른 작업이 완료 되면 대기 상태에 있던 작업을 이어서 처리하게 됩니다.

CPU에 부하를 주어 CPU 사용량을 테스트 했을 때, 일정 사용량을 넘지 않았습니다.
그리고 부하를 주는 프로그램의 로그 출력이 잠시나마 멈췄습니다.
