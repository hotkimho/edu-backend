...
### 서비스어카운트
쿠버네티스에선 API 서버에 접속하는 두 종류의 클라이언트를 구분합니다.
- 사람(사용자)
- 파드(파드 내부에서 실행되는 애플리케이션)

사용자 계정
- 사용자는 쿠버네티스 클러스터와 상호작용하는 사람을 나타냅니다 주로 클러스터 관리자, 개발자 등이 해당됩니다. 
- 사용자 계정은 `kubectl`과 같은 도구를 사용해서 API 서버와 상호작용합니다.

서비스 계정(서비스어카운트)
- 서비스 계정은 클러스터 내부에서 실행되는 어플리케이션이 쿠버네티스 API 서버와 상호작용할 수 있도록 하는 계정입니다.
- 클러스터의 네임스페이스에서 관리되며,  `서비스어카운트` 라는 리소스로 생성되고 저장됩니다.
  
그룹
- 사용자와 서비스어카운트는 하나 이상의 그룹에 속할 수 있습니다.
- 그룹은 개별 사용자에게 권한을 부여하지 않고 한 번에 여러 사용자에게 동일한 권한을 부여하는데 사용됩니다.
- 그룹은 이름은 문자열이지만 내장된 그룹(system:"이름")은 특별한 의미를 같습니다.
  - system:authenticated: 그룹은 성공적으로 인증된 사용자에게 할당됩니다.
  - 이런 식으로 특정 의미를 갖는 내장된 그룹이 존재

모든 네임스페이스는 기본 서비스어카운트가 있고 별도로 지정하지 않을 시 기본 서비스 서비스어카운트를 사용합니다. 

파드는 생성되면 `/var/run/secrets/kubernetes.io/serviceaccount/token` 서비스어카운트의 인증 정보가 생성됩니다. 애플리케이션이 이 인증정보를 사용해 API 서버에 접속하면 인증 플러그인이 서비스어카운트를 인증하고 서비스어카운트 이름을 API 서버에 코어로 전달합니다. 

`system:serviceaccount:<namespace>:<service account name>`
API 서버는 인가 플러그인(rabc)에 이 사용자 이름을 전달하며, 애플리케이션이 수행하려는 작업을 서비스어카운트에서 수행할 수 있는지 결정합니다.
