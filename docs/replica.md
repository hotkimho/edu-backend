...
### 오너 레퍼런스
특정 리소스가 다른 리소스에 의해 생성되면 `오너 레퍼런스`라는 메타데이터를 가지게 됩니다.
오너 레퍼런스는 리소스가 어느 상위 리소스에 의해 생성되었는지 추적할 수 있습니다. 상위 리소스가 컨트롤러 또는 가비지 컬렉션에 의해 삭제될 때 하위 리소스(디펜던트 리소스)를 먼저 삭제함을 보장합니다.
이는 `blockOwnerDeletion: True`로 설정되어 있어야 합니다. 

오너 레퍼런스 관계를 가지는 주요한 리소스는 다음과 같습니다
- Deployment
  - Replicaset
    - Pod
- DaemonSet
...
```
deployment를 상위 리소스를 가지고 있고, deployment로 부터 생성된 걸 확인할 수 있습니다.
