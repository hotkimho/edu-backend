...
- [서비스](#서비스)
  ...
  - [노드포트](#노드포트)
  - [외부 연결을 사용하면서 발생할 수 있는 문제(불필요흔 네트워크 홉)](#외부-연결을-사용하면서-발생할-수-있는-문제불필요흔-네트워크-홉)
  - [외부 연결을 사용하면서 발생할 수 있는 문제(클라이언트 IP 보존)](#외부-연결을-사용하면서-발생할-수-있는-문제클라이언트-ip-보존)
  - [로드밸런서](#로드밸런서)
  - [인그레스](#인그레스)
  - [인그레스 클래스](#인그레스-클래스)
  - [인그레스 컨트롤러](#인그레스-컨트롤러)
...
### 서비스 동기
파드도 내부적으로 IP를 가지고 있지만 쿠버네티스의 파드는 삭제되고 변경되기 쉽습니다. 외부에서 특정 파드의 IP를 알고 접속을 한다고 해도 IP가 변경될 수 있어 원하는 파드에 접근한건지 보장할 수 없습니다. 그리고 파드가 할당될 때 IP가 부여되므로 미리 파드의 IP를 알 수 없습니다.

서비스 리소스를 사용하여 파드 집합에 대한 로드밸런싱을 수행하여 파드는 파드의 IP변경(새로운 파드가 생성) 되어도 클러스터의 목표 상태를 보장하도록 합니다.
...
이렇게 되면 사용자는 프론트엔드 서비스의 정보(IP, DOMAIN)만 알면 되고, 프론트엔드 파드는 백엔드 서비스 정보만 알면 됩니다.

서비스의 IP는 고정된 IP를 사용하고, 포트는 서비스에 정의된 포트를 사용하여 사용자는 파드의 생성과 삭제에 따른 IP변경을 신경쓸 필요 없이 서비스로 요청하면 됩니다.
...
셀렉터 없이 사용하는 서비스는 `외부 리소스나 특정 IP주소로 트래픽을 전달`에 사용됩니다. 실제 사용 사례는 아래와 같습니다.
- 하나의 서비스에서 다른 클러스터의 서비스를 지정하려고 할 때
  - A, B 서비스가 있다고 가정할 때 A서비스에 엔드포인트에 B서비스로 요청을 전달하게 설정할 수 있습니다.
  - 이는 B 서비스가 직접적으로 외부에 노출되지 않을 때 유용하게 사용할 수 있으며, 네트워크 정책, 보안, 접근 제어등의 이유로 A 서비스를 통해 B서비스로 요청을 우회하도록 설정할 수 있습니다.
...
모든 파드는 기본적으로 `kube-dns` 서비스를 사용하도록 구성됩니다. 그리고 이 설정은 파드의 `/etc/resolv.conf` 파일에서 기본 DNS 서버가 kube-dns로 설정되어 있기 때문입니다. 파드에서 실행되는 모든 DNS 쿼리는 kube-dns로 전달됩니다. 이는 파드를 정의할 때 `dnsPolicy` 속성을 사용하여 kube-dns를 사용할지 말지를 설정할 수 있습니다.
...
헤드리스 서비스와, 서비스의 차이는 서비스에 클러스터 IP를 할당받지 않고 kube-proxy가 서비스를 처리하지 않습니다. 그래서 로드 밸런싱 또는 프록시를 사용하지 않습니다.
...
헤드리스 서비스 셀렉터를 사용하는 경우와 사용하지 않는 경우는 각각의 목적과 사용사례에 따라 다릅니다. 아래에 사례를 정리합니다.
...
셀렉터를 사용하지 않는 경우
- 클러스터 외부 서비스와 통신
  - 클러스터 외부에 있는 데이터베이스나 API 서비스에 접근해야 할 때 사용합니다.
- 클러스터 내부
  - statefulset 과 함께 사용하여 각 파드의 고유한 IP주소를 DNS를 통해 노출함으로써 파드 간 통신을 가능하게 합니다.
  - 이는 데이터베이스 클러스터에 많이 사용됩니다.
...
### 외부 연결을 사용하면서 발생할 수 있는 문제(불필요흔 네트워크 홉)
