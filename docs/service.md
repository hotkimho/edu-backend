...
이렇게 되면 사용자는 프론트엔드 서비스의 정보(IP, PORT, DOMAIN)만 알면 되고, 프론트엔드 파드는 백엔드 서비스 정보만 알면 됩니다.

서비스의 IP/PORT는 고정되어 있어야 하며, 파드의 생성과 삭제에 따른 IP 변경은 사용자는 신경쓸 필요 없이 서비스로 요청하면 됩니다. 
