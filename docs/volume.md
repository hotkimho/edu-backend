...
하지만 클러스터 관리자 입장에선 실제 스토리지를 pv에 바인딩일 미리 해놔야 합니다. 쿠버네티스는 동적 프로비저닝을 통해 이 작업을 자동으로 수행합니다.
...
2. pvc 요청
사용자가 pvc를 생성하고 요청합니다. pvc는 `필요한 스토리지 크기`와 `접근 모드`를 지정하고 사용할 스토리지 클래스틑 참조합니다.
...
실제로 pvc 요청이 오면 nfs-storage 클래스를 참조하여 실제 pv가 생성 및 할당이 된다.
